...
// --- MENU GLOBAL ---
// Certifique-se que o caminho está correto para sua estrutura de pastas
import { KingdomMenu } from "../components/KingdomMenu";

export const KingdomHeartsContact = () => {
  const [formStep, setFormStep] = useState(0); // Controla onde o cursor aponta
  ...
  const copyEmail = () => {
    navigator.clipboard.writeText("seu.email@exemplo.com"); // TROQUE PELO SEU EMAIL
    setCopied(true);
  ...
  return (
    <div className="relative w-full min-h-screen bg-[#020014] text-white font-sans overflow-hidden flex flex-col items-center justify-start pt-28 md:pt-36 selection:bg-blue-500 selection:text-white">
      {/* --- BACKGROUND TEMA KINGDOM HEARTS (DIVE TO THE HEART VIBE) --- */}
      <div className="absolute inset-0 z-0 overflow-hidden">
        ...
        {/* Luzes/Memórias etéreas subindo */}
        <Particles
          id="kh-particles"
          init={particlesInit}
          options={{
            ...
            particles: {
              ...
              color: { value: ["#4ea6ff", "#ff6ec7", "#ffd700", "#ffffff"] },
      ...
      <div className="relative z-30 w-full max-w-6xl p-6 grid md:grid-cols-2 gap-12 items-center">
        {/* --- LADO ESQUERDO: MENSAGEM (COMMAND MENU) --- */}
        <div className="order-2 md:order-1">
          {!isSent ? (
            ...
            >
                ...
                <div className="bg-black/50 rounded-lg p-6 md:p-8 space-y-6 relative overflow-hidden">
                  ...
                  >
                    ...
                    <div className="space-y-1 group relative">
                      <label className="text-xs uppercase tracking-widest text-blue-300 ml-1 group-focus-within:text-yellow-400 transition-colors font-bold">
                        Your Name
                      </label>
                      <input
                        type="text"
                        required
                        className="w-full bg-blue-950/30 border-b-2 border-blue-500/30 px-4 py-3 focus:outline-none focus:border-yellow-400 focus:bg-blue-900/40 transition-all rounded-t-sm text-lg text-white placeholder-blue-300/30"
                        placeholder="Sora..."
                    ...
                    <div className="space-y-1 group relative">
                      ...
                      <textarea
                        rows={4}
                        required
                        className="w-full bg-blue-950/30 border-b-2 border-blue-500/30 px-4 py-3 focus:outline-none focus:border-yellow-400 focus:bg-blue-900/40 transition-all rounded-t-sm text-lg text-white placeholder-blue-300/30 resize-none"
              ...
              {/* --- NOVO CURSOR DE CHAVE (SVG PEQUENO) --- */}
              <motion.div
                animate={{ y: formStep * 88 + 28 }} // Posição vertical ajustada
                className="absolute -left-10 top-20 hidden md:block z-50 filter drop-shadow-[0_0_6px_rgba(255,255,255,0.7)]"
                transition={{ type: "spring", stiffness: 250, damping: 20 }}
              >
                {/* SVG da Chave com tamanho reduzido (30x30) */}
                <svg
                  width="30"
                  height="30"
                  viewBox="0 0 24 24"
                ...
                >
                  <path d="M7 11L12 2L17 11H7Z" />
                  <rect x="10" y="11" width="4" height="8" />
                  <path d="M5 14C5 12.8954 5.89543 12 7 12H17C18.1046 12 19 12.8954 19 14V17C19 18.1046 18.1046 19 17 19H7C5.89543 19 5 18.1046 5 17V14Z" />
                </svg>
              </motion.div>
            </motion.div>
          ) : (
        ...
        <div className="order-1 md:order-2 flex flex-col justify-center items-center md:items-start text-center md:text-left space-y-8">
          ...
          >
            <h1
              className="text-5xl md:text-7xl font-bold tracking-tight text-white drop-shadow-[0_0_15px_rgba(0,100,255,0.8)]"
          ...
          <div className="grid gap-5 w-full max-w-sm">
            ...
            <motion.button
              ...
              whileTap={{ scale: 0.98 }}
              className="group flex items-center gap-4 bg-gradient-to-r from-blue-900/40 to-black/40 hover:from-blue-800/60 hover:to-blue-900/40 p-4 rounded-xl border-2 border-blue-500/30 hover:border-blue-400 transition-all cursor-pointer w-full shadow-lg relative overflow-hidden"
            >
